...
public class RoadmapService {
    ...
    public Roadmap generateAndSaveRoadmap(String loginId, RoadmapRequestDto request) {
        ...
        String prompt = String.format("""
                        ...
                        10. 자격증 선택 시 다음을 내부적으로 비교 검토하라.(출력 금지) 현재 조건에서의 상대적 합격 가능성, 준비 부담도, 선행 자격증이 이후 시험에 주는 긍정 효과, 전체 경로의 누적 학습 피로도. 단순 나열이 아니라 가장 안정적인 순서를 선택하라.
                        11.  **[Critical Rule]** 너의 답변은 시스템이 곧바로 파싱해야 하므로 반드시 아래의 JSON 규격으로만 출력해야 한다. 마크다운 기호(```json), 인삿말, 부가 설명 등은 절대 포함하지 말고 오직 JSON 텍스트만 반환하라. 스마트 따옴표 금지, 순자는 문자열이 아닌 정수형으로 출력, JSON 외 추가 텍스트 절대 금지.
